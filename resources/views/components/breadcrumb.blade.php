    ...
    <nav class="flex items-center gap-1.5 text-xs text-gray-400" aria-label="Breadcrumb">
        ...
        @foreach ($items as $item)
            ...
            @if (isset($item['url']) && !$loop->last)
                <a href="{{ $item['url'] }}" class="hover:text-indigo-600 transition-colors truncate max-w-[120px]">
                    {{ $item['label'] }}
                </a>
            @else
                <span class="text-gray-600 font-medium truncate max-w-[120px]">{{ $item['label'] }}</span>
            @endif
        @endforeach
    </nav>
@endif
