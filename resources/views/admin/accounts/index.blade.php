...
@section('breadcrumb', 'Kelola akun admin sistem')

@section('content')

<div class="mb-5 flex justify-end">
    <a href="{{ route('admin.accounts.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-brand-accent hover:bg-brand-accent/80 text-white text-sm font-bold rounded-xl shadow-lg shadow-black/10 transition transform active:scale-95">
...
@include('components.per-page-dropdown', ['route' => 'admin.accounts.index', 'current' => $admins->perPage()])
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand-light text-brand-secondary text-xs font-bold uppercase">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-sm text-gray-600 font-mono">{{ $admin->email }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            @if(!$admin->is_root_super_admin)
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <form id="delete-form-{{ $admin->id }}" method="POST" action="{{ route('admin.accounts.destroy', $admin) }}" class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                    <button type="button"
                                            onclick="if(confirm('Apakah Anda yakin ingin menghapus akun admin \'{{ addslashes($admin->name) }}\'?')) document.getElementById('delete-form-{{ $admin->id }}').submit();"
                            ...
                            @else
                                <span class="text-xs text-gray-400 italic">Dilindungi</span>
                            @endif
                        </td>
                    </tr>
                @empty
